...
def grafica_pases_por_equipo(n_clicks):
    ...
    fig.update_traces(
        ...
        # Personaliza el texto de hover
        hovertemplate="<b>%{y} pases</b><extra></extra>",
        selected=dict(marker=dict(color='crimson')),  # Cambia color en hover
        # Estilo cuando no está en hover
...
def update_grafica_cantidad_pases_jugada(n_clicks):
    ...
    fig = px.scatter(df, x='Momento', y='Pases', color='Equipo',
                     hover_data=['Fase_del_Partido'])
    return fig
